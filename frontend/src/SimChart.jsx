  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart data={series} margin={{ top: 8, right: 4, left: -18, bottom: 0 }}>
        ...
        <XAxis dataKey="label" tick={{ fontSize: 10, fill: 'var(--text-secondary)' }} stroke="var(--border)" minTickGap={40} tickLine={false} />
        <YAxis yAxisId="m" domain={[bottom, top]} tick={{ fontSize: 10, fill: 'var(--text-secondary)' }} stroke="var(--border)" tickLine={false} width={44} />
        <YAxis yAxisId="t" orientation="right" domain={['dataMin - 3', 'dataMax + 3']} tick={{ fontSize: 10, fill: 'var(--text-secondary)' }} stroke="var(--border)" tickLine={false} width={34} allowDecimals={false} />
        <Tooltip
          contentStyle={{
