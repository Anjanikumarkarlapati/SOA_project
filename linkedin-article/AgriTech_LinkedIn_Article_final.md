...
**Demo results.** The numbers below are **demo data**: an illustrative sample of 30 made up to show how responses map to design decisions. They are *not* collected survey responses.

![Demo data (illustrative, not real responses): how each survey answer tests an assumption behind the design.](upload-in-order/02-survey-demo.png)
*Demo data (illustrative, not real responses): how each survey answer tests an assumption behind the design.*

- **63%** (demo) irrigate on fixed days or by habit, and **57%** (demo) watered an already-wet field last season. This is the case for skipping runs automatically.
- **70%** (demo) saw crop stress before noticing dry soil. This is the case for a live health score and a 48-hour forecast.
- **87%** (demo) want plain-language advice, alone or with numbers. This is why recommendations are written as sentences.
- **83%** (demo) rate a remote emergency stop as important, and **67%** (demo) would trust an automatic skip. This is the case for the safety controls and the closed loop.

**How we addressed it:** each finding maps to a feature that is already built and tested: skip-if-moist, health scoring with a forecast, written recommendations, and the emergency stop with separate farmer and admin roles. Writing the survey before the code kept us honest, because every feature has a question that could prove it unnecessary.

**If you farm, or work with farmers, I'd love your real answers in the comments.** They will replace the demo data in the next iteration.
...
![The DTI cycle. Each stage produced a concrete requirement in the code.](upload-in-order/03-dti.png)
...
![One door in. Services find each other through Eureka, never by a hard-coded host or port.](upload-in-order/04-architecture.png)
...
![All five services registered and UP in the Eureka registry.](upload-in-order/05-eureka-registry.png)
...
![How a request earns trust, and the attacks we tested against.](upload-in-order/06-security.png)
...
![Operations dashboard: field health, sensor network status and a live alert feed.](upload-in-order/07-app-dashboard.png)
*Operations dashboard: field health, sensor network status and a live alert feed.*

![Crop monitoring: each field scored against its own optimal band, with trend lines.](upload-in-order/08-app-crops.png)
*Crop monitoring: each field scored against its own optimal band, with trend lines.*

![Irrigation control: schedules that skip a run when the soil is already wet enough.](upload-in-order/09-app-irrigation.png)
...
![Results from the running stack. Every check can be re-run from the repository.](upload-in-order/10-results.png)
